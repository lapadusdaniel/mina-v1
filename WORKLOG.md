...
### 2026-03-25 — Fix iOS save + calitate imagini în Client Gallery

- Investigat `src/components/ClientGallery.jsx` și confirmat că grila folosea deja `<img>`, dar sursa lor venea prin `blob:` URLs generate local din `mediaService.getPhotoUrl(...)`, ceea ce afecta long-press save pe iOS
- Adăugat în componentă un builder local pentru URL-uri reale către Worker, astfel încât imaginile din grilă să fie servite ca URL HTTP direct, nu ca `blob:` local
- Trecut grid-ul masonry pe varianta `medium` în toate layout-urile, eliminând fallback-ul anterior la `thumb` pentru layout-ul de `4col`
- Actualizat lightbox-ul să folosească `original` ca sursă principală pentru afișare, cu fallback la cache local dacă URL-ul direct nu este disponibil; download-ul a rămas pe `original`
- Setat overlay-ul de pe item-uri cu `pointer-events: none` și containerul de acțiuni cu `pointer-events: auto`, astfel încât long-press-ul să ajungă pe imagine pe iOS fără să strice butoanele existente
- Rulate cu succes `npm run build` și `FIREBASE_PROJECT_ID=\"mina-v1-aea51\" npm run deploy:hosting`
